...
class MyFrame(wx.Frame):
	...
	def __init__(self, parent, title, pos, size):
		...
		# The Enter Frequency Bouton
		self.enterXBouton = wx.Button(self.panel, id=-1, label="PULSATION", pos=(140, 75))
		self.enterXBouton.Bind(wx.EVT_BUTTON, self.enterFrequency)

		# The Frequency Slider
		self.frequencyText = wx.StaticText(self.panel, id=-1, label="Pulsation : 150", pos=(20, 80), size=wx.DefaultSize)
		self.frequency = wx.Slider(self.panel, id=-1, value=150, minValue=20, maxValue=6000, pos=(20, 60), size=(250, -1))
	...
	def enterFrequency(self, evt):
		x = evt.GetInt()
		saveBox = wx.TextEntryDialog(None, "Pulsation ?", "", "440")
		if saveBox.ShowModal()==wx.ID_OK:
			saveX = saveBox.GetValue()
			self.setFrequency(int(saveX))

	def setFrequency(self, x):
		self.f = x
		self.frequencyText.SetLabel("Pulsation : %d" %self.f)
		self.frequency.SetValue(x)
		self.W = self.f
		self.updateReponseForcee()
