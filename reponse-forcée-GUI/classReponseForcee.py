...
class Modes_Forces:
	...
	def calcule_reponse(self, t, W):
		...
		self.reponse = self.empty_reponse*0
	...
	"""def calcule_reponse2(self, t, W, amp, Xmn):
					reponse = self.empty_reponse
			
					for n in xrange(self.N):
						for m in xrange(self.M):
							reponse = X.get_mode(n,m)*amp.get_amp(n,m)*np.cos(W*t)
							self.reponse_index.append(reponse)
...
class Amplitude_forcee:
	...
	def compute_amps(self):
		self.amp_index = np.zeros((self.M, self.N))
